...
Library for detecting platform specific user
directories e.g. for data, config, cache, logs.

Most stuff is copied from the python library
...
type _Platform is
  (_Osx | _OsxAsUnix | _Unix | _Windows)

primitive Paths
  """
  Utility for joining sequences of path segments.
  """
  fun join(paths: ReadSeq[String]): String =>
    """
    Join a sequence of path segments into a single path.
    """
    Iter[String](paths.values())
      .fold[String](
        "", {(acc, cur) => Path.join(acc, cur) })
...
class AppDirs
  """
  Detects platform-specific user directories for
  data, config, cache, logs, and state.

  Provides methods that return the appropriate
  directory paths for the current platform (Windows,
  macOS, or Linux/Unix) based on the application name
  and optional author, version, and roaming settings.
  """
  ...
  =>
    """
    An AppDirs instance derives the platform specific
    directories from the process environment variables.
    It will return appdirs for the user of the current
    process.

    Creating appdirs requires:
     - passing environment variables, only on unix
       and osx systems.
     - a name of the app you want to get directories
       for
     - optionally: a name of the app author (be it a
       company or a person)
     - optionally: a version of your app, if you want
       to separate directories also by version
    """
  ...
  fun user_data_dir(): String ? =>
    """
    Return the full path to the user-specific data
  ...
  fun site_data_dirs(): Array[String] val ? =>
    """
    Return an array of full paths to the user-shared
    ...
    | let v: String =>
      for i in Range[USize](
        0, os_specific_dirs.size())
      do
        os_specific_dirs(i)? =
          Path.join(os_specific_dirs(i)?, v)
      end
    end
    os_specific_dirs

  fun _unix_site_data_dirs()
    : Array[String] iso^ ?
  =>
    let data_dirs: Array[String] iso =
      try
        Path.split_list(
          _env_vars("XDG_DATA_DIRS")?)
      else
    ...
    for idx in Range[USize](
      0, data_dirs.size())
    do
      data_dirs(idx)? =
        Path.join(data_dirs(idx)?, _app_name)
    end
    consume data_dirs
  ...
  fun site_config_dirs(): Array[String] val ? =>
    """
    Return full path to the user-shared config dirs
    for this application.
    """
    let os_specific_dirs: Array[String] iso =
      match \exhaustive\ _platform
  ...
  fun _unix_site_config_dirs()
    : Array[String] iso^ ?
  =>
    let config_dirs: Array[String] iso =
      try
        Path.split_list(
          _env_vars("XDG_CONFIG_DIRS")?)
      else
        // fallback default dirs
    ...
    do
      config_dirs(idx)? =
        Path.join(config_dirs(idx)?, _app_name)
    end
    consume config_dirs
