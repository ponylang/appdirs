...
use @SHGetKnownFolderPath[U32](
  rfid: Pointer[U8] tag, flags: U32,
  token: Pointer[U32],
  path: Pointer[Pointer[U16]]) if windows
use @WideCharToMultiByte[I32](
  code_page: U32, flags: U32,
  char_str: Pointer[U16],
  char_str_size: I32,
  multi_byte_str: Pointer[U8] tag,
  multi_byte_str_size: I32,
  default_char: Pointer[U8],
  used_default_char: Pointer[U8]) if windows
use @CoTaskMemFree[None](
  pv: Pointer[U16] tag) if windows
...
primitive KnownFolderIds
  ...
  Functions return the little endian byte values of the
  folderid GUIDs.
  """
  fun profile(): Array[U8] val =>
    ...
    Applications should not create files or folders at
    this level; they should put their data under the
    locations referred to by CSIDL_APPDATA or
    CSIDL_LOCAL_APPDATA.
    However, if you are creating a new Known Folder
    the profile root referred to by CSIDL_PROFILE
    is appropriate.
  ...
  fun program_data(): Array[U8] val =>
    ...
    data for all users. A typical path is
    C:\Documents and Settings\All
    Users\Application Data. This folder is used for
    application data that is not user specific. For
    example, an application can store a spell-check
    dictionary, a database of clip art, or a log file
    in the CSIDL_COMMON_APPDATA folder. This
    information will not roam and is available to anyone
    using the computer.
...
primitive KnownFolders
  ...
  fun apply(folderid: Array[U8] val): String iso^ ? =>
    """
    Look up a Windows known folder path by its GUID.

    Returns the folder path as a UTF-8 string. Raises
    an error if the folder cannot be resolved.
    """
    ifdef not windows then
      ...
      let utf8_path: Array[U8] iso =
        recover Array[U8](bytes_necessary.usize()) end
      utf8_path.undefined(bytes_necessary.usize())
      // second call to perform conversion
      let convert_result: I32 =
        @WideCharToMultiByte(
          WindowsCodePages.utf8(),
          U32(0),
          path_pointer,
